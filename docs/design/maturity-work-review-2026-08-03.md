...
Current gap: every dashboard registry item still reports `production commit not recorded`. Most also report `deployment source note missing`.
...
- Add a promotion ledger generated by the Hetzner promotion script.
- Record service, target ref, resolved commit SHA, previous commit SHA, timestamp, operator, health result, proof URL result, and rollback command.
- Either update `hermes.dashboards.json` after each deploy or write a separate append-only deployment evidence ledger consumed by `dashboard-deployment-ledger`.
...
1. V14: Deployment Source Ledger
   - Make promotion scripts write append-only deploy evidence.
   - Teach Nous deployment ledger to consume actual production records.
