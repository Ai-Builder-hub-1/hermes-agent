...
export function buildCapacityCommandCenterSnapshot(input: CapacityCommandCenterInput): DashboardSnapshotContract {
...
export function buildSampleCapacityCommandCenterSnapshot(generatedAt = "2026-07-24T00:00:00.000Z"): DashboardSnapshotContract {
...
function buildCapacityModules(input: CapacityCommandCenterInput, missingFeeds: string[]): DashboardModuleContract[] {
  return [
...
function buildCapacityAlerts(input: CapacityCommandCenterInput, totals: CapacityTotals): DashboardAlertContract[] {
  const alerts: DashboardAlertContract[] = [];
...
function buildCostContracts(input: CapacityCommandCenterInput): DashboardCostContract[] {
...
function calculateCapacityTotals(input: CapacityCommandCenterInput): CapacityTotals {
...
function breakdown(
  ...
  storageGb: number,
  status: DashboardOperationalStatus,
  trend: CapacityCommandCenterBreakdown["trend"],
  dataTrust: CapacityCommandCenterBreakdown["dataTrust"],
): CapacityCommandCenterBreakdown {
...
function module(
  id: string,
  label: string,
  workspace: DashboardModuleContract["workspace"],
  primaryQuestion: string,
  input: CapacityCommandCenterInput,
  status: DashboardOperationalStatus,
): DashboardModuleContract {
  return {
    id,
...
function metric(
  id: string,
  label: string,
  value: DashboardMetricContract["value"],
  unit: string,
  detail: string,
  status: DashboardOperationalStatus,
): DashboardMetricContract {
...
function budgetStatus(input: CapacityCommandCenterInput): DashboardOperationalStatus {
  if (typeof input.budgetUsd !== "number" || typeof input.budgetUsedUsd !== "number" || input.budgetUsd <= 0) return "unknown";
  const percentUsed = input.budgetUsedUsd / input.budgetUsd;
  if (percentUsed >= 1) return "blocked";
  if (percentUsed >= 0.85) return "degraded";
  if (percentUsed >= 0.65) return "watch";
  return "healthy";
}

function worstFromBreakdowns(breakdowns: CapacityCommandCenterBreakdown[]): DashboardOperationalStatus {
  if (breakdowns.some((entry) => entry.status === "blocked")) return "blocked";
  if (breakdowns.some((entry) => entry.status === "degraded")) return "degraded";
  if (breakdowns.some((entry) => entry.status === "watch")) return "watch";
  if (breakdowns.some((entry) => entry.status === "unknown")) return "unknown";
  return "healthy";
}

function sum(items: CapacityCommandCenterBreakdown[], key: "tokensIn" | "tokensOut"): number {
  return items.reduce((total, item) => total + item[key], 0);
}

function sumOptional(items: CapacityCommandCenterBreakdown[], key: "estimatedCostUsd" | "revenueUsd" | "apiCalls" | "storageGb"): number {
