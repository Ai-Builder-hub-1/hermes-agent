...
function isKnownWorkspace(workspace: string): workspace is DashboardWorkspaceId {
...
function validateDataSource(source: DashboardDataSourceState, path: string, issues: DashboardValidationIssue[]) {
  if (!source.id) pushIssue(issues, "error", "data-source.id.missing", `${path}.id`, "Data source must have an id.");
  if (!source.label) pushIssue(issues, "error", "data-source.label.missing", `${path}.label`, "Data source must have a label.");
  if (!source.owner) pushIssue(issues, "error", "data-source.owner.missing", `${path}.owner`, "Data source must declare an owner.");
  if (!source.status) pushIssue(issues, "error", "data-source.status.missing", `${path}.status`, "Data source must declare status.");
...
function validateModule(module: DashboardModuleContract, index: number, issues: DashboardValidationIssue[]) {
  const path = `modules[${index}]`;
  if (!module.id) pushIssue(issues, "error", "module.id.missing", `${path}.id`, "Module must have an id.");
  if (!module.label) pushIssue(issues, "error", "module.label.missing", `${path}.label`, "Module must have a label.");
...
export function validateDashboardSnapshot(snapshot: DashboardSnapshotContract): DashboardValidationResult {
  const issues: DashboardValidationIssue[] = [];

  if (!snapshot.id) pushIssue(issues, "error", "snapshot.id.missing", "id", "Snapshot must have an id.");
  if (!snapshot.projectId) pushIssue(issues, "error", "snapshot.project-id.missing", "projectId", "Snapshot must have a project id.");
  if (!snapshot.generatedAt) pushIssue(issues, "error", "snapshot.generated-at.missing", "generatedAt", "Snapshot must have a generated timestamp.");
  if (!snapshot.modules.length) pushIssue(issues, "error", "snapshot.modules.empty", "modules", "Snapshot must include dashboard modules.");
...
export function validateDashboardAdoptionRegistry(registry: DashboardAdoptionRegistry): DashboardValidationResult {
  const issues: DashboardValidationIssue[] = [];
  ...
  registry.dashboards.forEach((dashboard, index) => {
    ...
    if (!dashboard.project) pushIssue(issues, "error", "adoption.project.missing", `${path}.project`, "Dashboard adoption entry must have a project.");
    if (!dashboard.name) pushIssue(issues, "error", "adoption.name.missing", `${path}.name`, "Dashboard adoption entry must have a name.");
    if (!dashboard.type) pushIssue(issues, "error", "adoption.type.missing", `${path}.type`, "Dashboard adoption entry must have a type.");
    if (!dashboard.status) pushIssue(issues, "error", "adoption.status.missing", `${path}.status`, "Dashboard adoption entry must have a status.");
    if (!dashboard.targetState) pushIssue(issues, "error", "adoption.target-state.missing", `${path}.targetState`, "Dashboard adoption entry must describe target state.");
