...
const backlogMarkdownPath = path.join(root, "docs/design/world-class-dashboard-system-backlog.md");
...
function createBacklogMarkdown(report, backlog) {
  const openItems =
    backlog.items.map((item) => `- ${item.layerName}: ${item.title}`).join("\n") ||
    "- No open world-class dashboard system items.";
  return `# World-Class Dashboard System Backlog
...
As of the latest report, the central system scores \`${report.score}/100\` and is \`${report.status}\`.

Open items: \`${backlog.openItems}\`.
...
let backlog = null;
if (writeBacklog) {
  backlog = createBacklog(report);
  fs.writeFileSync(backlogPath, `${JSON.stringify(backlog, null, 2)}\n`);
  fs.writeFileSync(backlogMarkdownPath, createBacklogMarkdown(report, backlog));
}
...
if (json) {
  ...
  if (writeBacklog) {
    console.log(`Wrote backlog: docs/design/world-class-dashboard-system-backlog.json`);
    console.log(`Wrote backlog summary: docs/design/world-class-dashboard-system-backlog.md`);
  }
}
