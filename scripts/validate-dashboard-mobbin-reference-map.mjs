...
if (!issues.some((item) => item.severity === "error")) {
  ...
  if (map.version !== "V12") issue("error", "Mobbin reference map must declare version V12.");
  ...
  if ((map.references ?? []).length < 21) issue("error", "Mobbin reference map must cover the expanded V12 reference set.");
  const requiredPatterns = [
    ...
    "business-mapping-workspace"
  ];
  const coveredPatterns = new Set((map.references ?? []).flatMap((reference) => reference.patterns ?? []));
  for (const pattern of requiredPatterns) {
    if (!coveredPatterns.has(pattern)) issue("error", "Mobbin reference map is missing required dashboard family coverage.", pattern);
  }
